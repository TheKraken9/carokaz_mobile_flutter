...
class BoiteService {
  ...
  Future<APIResponse<List<BoiteModel>>> getAllBoite() async {
    ...
    final response = await ioClient.get(Uri.parse('$baseurl/boites'), headers: headers);
    ...
    if (response.statusCode == 200) {
      ...
      // Mappez les objets JSON en objets BoiteModel
      final boites = data.map((item) => BoiteModel.fromJson(item)).toList();

      // Retournez une APIResponse avec les données des marques
      return APIResponse<List<BoiteModel>>(data: boites);
    } else {
      // Gérer les erreurs HTTP ici
      return APIResponse<List<BoiteModel>>(
